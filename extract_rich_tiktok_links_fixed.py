...
def clean_and_normalize_video_url(url):
    """Clean and normalize a TikTok or Instagram Reels URL to remove corrupted data."""
    if not url:
        return None
    ...
    try:
        ...
        # Extract different Instagram Reels URL patterns
        instagram_patterns = [
        ...
        for pattern in all_patterns:
            ...
            if match:
                ...
                # Normalize TikTok URLs to standard format
                ...
                # Normalize Instagram URLs to standard format
                elif 'instagram.com' in clean_url:
                    clean_url = clean_url.replace('m.instagram.com', 'www.instagram.com')
                    # Ensure it ends with / for consistency
                    if not clean_url.endswith('/'):
                        clean_url += '/'
...
def extract_urls_from_binary(binary_data):
    """Extract TikTok and Instagram Reels URLs from binary data (attributedBody BLOB)."""
    if not binary_data:
        return set()
    ...
    try:
        ...
        # Look for TikTok and Instagram URLs in the decoded text
        # Use a more comprehensive pattern to catch various formats
        video_pattern = r'https?://[^\s]*(?:tiktok\.com|vm\.tiktok\.com|instagram\.com)[^\s]*'
        
        matches = re.findall(video_pattern, text_data)
        
        for match in matches:
            # Clean each match
            clean_url = clean_and_normalize_video_url(match)
            if clean_url:
                urls.add(clean_url)
...
def extract_rich_video_links(days_back=60, from_sender=None):  # Extended default to 60 days
    video_links = {}  # Use a dictionary to store unique links, keyed by URL
    ...
    try:
        ...
        for row in rows:
            ...
            if text_content:
                clean_url = clean_and_normalize_video_url(text_content)
                if clean_url:
            ...
            # Add found URLs to the main video_links dictionary
            # Keep only the most recent timestamp for each URL
            for url in current_message_urls:
                if url not in video_links or timestamp > video_links[url]['timestamp']:
                    video_links[url] = {
                        'url': url,
                        'timestamp': timestamp,
        ...
        sorted_links = sorted(video_links.values(), key=lambda x: x['timestamp'], reverse=True)
        final_links = []
        ...
        print(f"Successfully extracted {len(final_links)} unique video links to {output_file}")
...
if __name__ == "__main__":
    ...
    extract_rich_video_links(days_back=60, from_sender=girlfriend_number) 